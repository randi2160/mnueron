...
use the `mnueron` CLI or the MCP server directly. This SDK requires a hosted
backend URL + API token.

    pip install mnueron

    from mnueron import Mnueron
    with Mnueron(api_key="mn_xxx") as client:
...
class Mnueron:
    ...
    def __init__(
        ...
        base_url = (base_url or os.getenv("MNUERON_API_URL") or "https://api.mnueron.dev").rstrip("/")
        self._client = httpx.Client(
            base_url=base_url,
            headers={
                "Authorization": f"Bearer {api_key}",
                "User-Agent": "mnueron-python/0.1",
            },
            timeout=timeout,
        )
    ...
    def _check(self, r: httpx.Response) -> Any:
        if r.status_code >= 400:
            raise MnueronError(r.status_code, r.text or r.reason_phrase)
...
class AsyncMnueron:
    """Asyncio client. Use ``async with AsyncMnueron(...) as client:`` for cleanup."""
...
__all__ = ["Mnueron", "AsyncMnueron", "Memory", "Namespace", "BulkResult", "MnueronError"]
